...
class CoverageStatisticsHtmlPrinter:
    ...
    def create_coverage_html_file(coverage_statistics, html_compare_dir, html1_dir, html2_dir):
        ...
        if not len(coverage_statistics.counters):
            return
        html_file_name = CoverageStatisticsHtmlPrinter.__get_html_file_name(coverage_statistics)
        ...
        for coverage_type in CoverageStatisticsHtmlPrinter.counter_types.values():
        ...
        html_table_foot = '<tfoot><tr><td>Total</td>' + CoverageStatisticsHtmlPrinter.__get_html_table_line(coverage_statistics, summarize=True) + '</tr></tfoot>'
        ...
        for child_statistics in coverage_statistics.children:
            ...
            child_html_file = CoverageStatisticsHtmlPrinter.__get_html_file_name(child_statistics)
            html_table_line = '<tr>'
            href = ''
            if child_html_file:
        ...
        table_legend = '<table><tr>' \
                       '<td class=bar>' + DiffCounterHtmlPrinter.get_html_bar_segment(1, 'red', 10) + '</td>' \
                       '<td> Missed by both test suites</td></tr>' \
                       '<tr><td class=bar>' + DiffCounterHtmlPrinter.get_html_bar_segment(1, 'gold', 10) + '</td>' \
                       '<td> Missed by first test suite only</td></tr>' \
                       '<tr><td class=bar>' + DiffCounterHtmlPrinter.get_html_bar_segment(1, 'blue', 10) + '</td>' \
                       '<td> Missed by second test suite only</td></tr>' \
                       '<tr><td class=bar>' + DiffCounterHtmlPrinter.get_html_bar_segment(1, 'green', 10) + '</td>' \
    ...
    def __get_html_table_line(coverage_statistics, summarize=False):
        ...
        '''
        line = ''
        for coverage_type, coverage_type_data in CoverageStatisticsHtmlPrinter.counter_types.items():
            if coverage_statistics.counters.get(coverage_type) and coverage_statistics.counters[coverage_type].total != 0:
                ...
                if coverage_type_data['is_bar']:
                    if not summarize:
                        ...
                        line += DiffCounterHtmlPrinter.get_html_bar(counter, max_val)
                    else:
                        line += DiffCounterHtmlPrinter.get_html_coverage_diff(counter)
                else:
                    line += DiffCounterHtmlPrinter.get_html_misses_diff(counter)
            else:
...
class DiffCounterHtmlPrinter:
    ...
    def get_html_bar(counter, max_val):
        ...
        ibar = DiffCounterHtmlPrinter.get_html_bar_segment(bar_scale, 'red', counter.missed_both)
        ibar += DiffCounterHtmlPrinter.get_html_bar_segment(bar_scale, 'gold', counter.missed_only1)
        ibar += DiffCounterHtmlPrinter.get_html_bar_segment(bar_scale, 'blue', counter.missed_only2)
        ibar += DiffCounterHtmlPrinter.get_html_bar_segment(bar_scale, 'green', counter.missed_none)
        ibar += DiffCounterHtmlPrinter.__get_html_coverage_compare(counter)
